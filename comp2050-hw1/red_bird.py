...
from game_engine.game_rules import ClassicGameRules
...
def read_command():
    ...
    EXAMPLES:   (1) python red_bird.py -p MinimaxAgent -l aiAdversarial -a depth=4 -b GreedyBlackBirdAgent --frame_time 0.05
                        will start an adversarial game with your MinimaxAgent vs the GreedyBlackBirdAgent
                        on the aiAdversarial level
                (2) python red_bird.py -l aiAdversarial -p KeyboardAgent -b GreedyBlackBirdAgent
    ...
    if options.quiet_graphics:
        from game_engine.text_display import NullGraphics
        args['display'] = NullGraphics()
    elif options.text_graphics:
        from game_engine.text_display import SLEEP_TIME, RedBirdGraphics
        SLEEP_TIME = options.frame_time
        args['display'] = RedBirdGraphics()
    else:
        from game_engine.graphics_display import RedBirdGraphics
        args['display'] = RedBirdGraphics(options.zoom,
